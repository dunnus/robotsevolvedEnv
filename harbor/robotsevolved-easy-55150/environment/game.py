#!/usr/bin/env python3
"""Offline CLI and verifier engine for one sliding-robot puzzle."""
...
def move(puzzle: dict, robots: dict[str, list[int]], token: str) -> bool:
    if len(token) != 2 or token[0] not in COLORS or token[1] not in DIRECTIONS:
        raise ValueError(f"invalid move {token!r}; expected robot B/G/R/Y plus N/E/S/W")
    ...
    while True:
        blocked = (
            (direction == "N" and (row == 0 or (row, col) in horizontal))
...
def simulate(puzzle: dict, moves: list[str]) -> tuple[bool, int, dict[str, list[int]]]:
    ...
    valid = 0
    for token in moves:
        if move(puzzle, robots, token.upper()):
            valid += 1
...
def reward(puzzle: dict, moves: list[str]) -> float:
    ...
    if solved and len(moves) <= puzzle["max_moves"]:
        return max(0.5, 1.0 - 0.02 * max(0, len(moves) - 7))
...
def show(puzzle: dict) -> None:
    ...
    print(f"goal={puzzle['goal']} max_moves={puzzle['max_moves']}")
...
def main() -> int:
    ...
    if command == "show":
        ...
        return 0
    if command == "test":
        moves = [move.strip().upper() for move in sys.argv[2].split(",") if move.strip()]
        solved, valid, robots = simulate(puzzle, moves)
        print(json.dumps({"solved": solved, "reward": reward(puzzle, moves), "valid_moves": valid, "robots": robots}))
        return 0
    if command == "evaluate":
        ...
        try:
            ...
            if not isinstance(moves, list) or not all(isinstance(move, str) for move in moves):
                raise ValueError("solution must be a JSON array of strings")
            value = reward(puzzle, moves)
            print(value)
